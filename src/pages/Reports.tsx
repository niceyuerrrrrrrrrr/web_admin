  ...
  const columns: ColumnsType<ReportRecord> = [
    {
      title: '标题',
      dataIndex: 'title',
      filters: Array.from(new Set((reportsQuery.data?.records || []).map(r => r.title).filter(Boolean)))
        .sort()
    ...
    },
    {
      title: '类型',
      dataIndex: 'type',
      filters: Array.from(new Set((reportsQuery.data?.records || []).map(r => r.type).filter(Boolean)))
        .sort()
        .map(val => ({ text: val, value: val })),
    ...
    },
    {
      ...
      filters: Array.from(new Set((reportsQuery.data?.records || []).map(r => r.current_approver).filter(Boolean)))
        .sort()
    ...
    },
    {
      ...
      filters: Array.from(new Set((reportsQuery.data?.records || []).map(r => r.applicant_name).filter(Boolean)))
        .sort()
        .map(val => ({ text: val as string, value: val as string })),
