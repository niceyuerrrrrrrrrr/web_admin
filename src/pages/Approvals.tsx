    ...
    () => [
      {
        title: '审批类型',
        dataIndex: 'type_name',
        filters: Array.from(new Set((pendingQuery.data?.records || []).map(item => item.type_name).filter(Boolean)))
          .sort()
      ...
      },
      {
        title: '申请人',
        dataIndex: 'user_name',
        filters: Array.from(new Set((pendingQuery.data?.records || []).map(item => item.user_name).filter(Boolean)))
          .sort()
          .map(val => ({ text: val, value: val })),
      ...
      },
      {
        ...
        filters: Array.from(new Set((pendingQuery.data?.records || []).map(item => item.current_approver).filter(Boolean)))
          .sort()
    ...
    () => [
      {
        title: '审批类型',
        dataIndex: 'type_name',
        filters: Array.from(new Set((historyQuery.data?.records || []).map(item => item.type_name).filter(Boolean)))
          .sort()
          .map(val => ({ text: val, value: val })),
        onFilter: (value, record) => record.type_name === value,
        width: 140,
      ...
      },
      {
        title: '申请人',
        dataIndex: 'user_name',
        filters: Array.from(new Set((historyQuery.data?.records || []).map(item => item.user_name).filter(Boolean)))
          .sort()
          .map(val => ({ text: val, value: val })),
        onFilter: (value, record) => record.user_name === value,
        width: 120,
      ...
      },
      {
        title: '当前节点审批人',
        dataIndex: 'current_approver',
        filters: Array.from(new Set((historyQuery.data?.records || []).map(item => item.current_approver).filter(Boolean)))
          .sort()
          .map(val => ({ text: val as string, value: val as string })),
        onFilter: (value, record) => record.current_approver === value,
        width: 150,
