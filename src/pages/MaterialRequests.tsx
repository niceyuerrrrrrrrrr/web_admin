        ...
        render: (_, record) => (
          <Space>
            ...
            {record.status === 'submitted' && record.user_id === (user as any)?.id && (
              <Button type="link" icon={<CheckCircleOutlined />} loading={submitMutation.isPending} onClick={() => submitMutation.mutate(record.id)}>
                提交审批
              </Button>
            )}
            {record.status === 'reviewing' && canApprove && (
              <>
                <Button type="link" icon={<CheckCircleOutlined />} onClick={() => setActionModal({ type: 'approve' })}>
                  通过
                </Button>
                <Button type="link" danger icon={<CloseCircleOutlined />} onClick={() => setActionModal({ type: 'reject' })}>
                  拒绝
                </Button>
              </>
            )}
