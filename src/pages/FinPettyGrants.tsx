...
const FinPettyGrantsPage = () => {
  ...
  const [selectedRecord, setSelectedRecord] = useState<PettyGrantRecord | null>(null)
  const [createForm] = Form.useForm()

  const listQuery = useQuery({
    queryKey: ['fin', 'petty-grants', filters, currentPage, pageSize, effectiveCompanyId],
    queryFn: async () => {
      const params: any = {
        page: currentPage,
        page_size: pageSize,
  ...
  const createMutation = useMutation({
    mutationFn: async (values: any) => {
      ...
      const response = await client.post('/fin/petty-grants', {
  ...
  const openDetail = (record: PettyGrantRecord) => {
  ...
  const columns: ColumnsType<PettyGrantRecord> = [
    {
      title: 'ID',
      dataIndex: 'id',
      key: 'id',
  ...
  return (
    <div>
      ...
      >
        ...
        >
          ...
          <Form.Item label="用途" name="purpose">
            <Input.TextArea rows={3} placeholder="请输入备用金用途" />
          </Form.Item>
      ...
      {/* 详情抽屉 */}
      <Drawer
        title={`备用金发放详情 #${selectedRecord?.id || ''}`}
      ...
      >
        {selectedRecord && (
          <Descriptions bordered size="small" column={1}>
            ...
            <Descriptions.Item label="领用人">{selectedRecord.person_name}</Descriptions.Item>
            <Descriptions.Item label="发放金额(元)">
              <Text strong>¥{((selectedRecord.amount_cents || 0) / 100).toFixed(2)}</Text>
            </Descriptions.Item>
            <Descriptions.Item label="发放日期">{selectedRecord.grant_date}</Descriptions.Item>
...
export default FinPettyGrantsPage
