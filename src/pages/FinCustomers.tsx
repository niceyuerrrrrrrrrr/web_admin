...
import { createFinCustomer, fetchFinCustomers, toggleFinCustomerActive, updateFinCustomer } from '../api/services/finBase'
...
const yuanToCentsNullable = (yuan: any): number | null => {
...
const FinCustomersPage = () => {
  ...
  const [editing, setEditing] = useState<FinCustomerRecord | null>(null)
  ...
  const listQuery = useQuery({
    queryKey: ['fin', 'customers', filters, effectiveCompanyId],
    queryFn: () =>
      fetchFinCustomers({
  ...
  const createMutation = useMutation({
    mutationFn: createFinCustomer,
    onSuccess: () => {
      message.success('已创建')
      setCreateOpen(false)
      createForm.resetFields()
      queryClient.invalidateQueries({ queryKey: ['fin', 'customers'] })
    },
    onError: (e) => message.error((e as Error).message || '创建失败'),
  })

  const updateMutation = useMutation({
    mutationFn: (payload: { id: number; data: any }) => updateFinCustomer(payload.id, payload.data),
    onSuccess: () => {
      message.success('已更新')
      setEditOpen(false)
  ...
  const toggleMutation = useMutation({
    mutationFn: (payload: { id: number; isActive: number }) =>
      toggleFinCustomerActive(payload.id, { is_active: payload.isActive, companyId: effectiveCompanyId }),
    onSuccess: () => {
  ...
  const columns: ColumnsType<FinCustomerRecord> = useMemo(
    () =>
      addResizableToColumns<FinCustomerRecord>([
        {
          title: 'ID',
          dataIndex: 'id',
          key: 'id',
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  editForm.setFieldsValue({
                    ...
                    credit_limit_yuan: r.credit_limit_cents == null ? '' : centsToYuan(r.credit_limit_cents),
                    remark: r.remark,
    ...
    <div>
        ...
        <Card>
          ...
          >
            <Form.Item label="关键字" name="keyword">
              <Input placeholder="客户名称" style={{ width: 200 }} allowClear />
            </Form.Item>
      ...
      >
        <Form
          ...
          onFinish={(values) => {
            createMutation.mutate({
              ...
              credit_limit_cents: yuanToCentsNullable(values.credit_limit_yuan),
        ...
        >
          ...
          <Form.Item label="授信额度(元)" name="credit_limit_yuan">
...
export default FinCustomersPage
