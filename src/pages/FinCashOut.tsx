...
import type { FinAccountRecord, FinCashOutRecord, FinSupplierRecord } from '../api/types'
...
const FinCashOutPage = () => {
  ...
  const [selectedRecord, setSelectedRecord] = useState<FinCashOutRecord | null>(null)
  ...
  const [payeeType, setPayeeType] = useState<string>('other')
  ...
  const listQuery = useQuery({
    queryKey: ['fin', 'cash-out', filters, currentPage, pageSize, effectiveCompanyId],
    queryFn: () =>
      fetchCashOutList({
  ...
  const detailQuery = useQuery({
    queryKey: ['fin', 'cash-out', 'detail', selectedRecord?.id, effectiveCompanyId],
    queryFn: () => fetchCashOutDetail(selectedRecord!.id, { companyId: effectiveCompanyId }),
    enabled: !!selectedRecord && detailDrawerOpen,
  })

  const timelineQuery = useQuery({
    queryKey: ['fin', 'cash-out', 'timeline', selectedRecord?.id],
    queryFn: () => fetchApprovalTimeline('fin_cash_out', selectedRecord!.id),
    enabled: !!selectedRecord && detailDrawerOpen,
  })
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      message.success('付款单已创建')
      createForm.resetFields()
      setCreateModalOpen(false)
      queryClient.invalidateQueries({ queryKey: ['fin', 'cash-out'] })
    },
    onError: (error) => {
  ...
  const submitMutation = useMutation({
    mutationFn: (id: number) => submitCashOut(id, { companyId: effectiveCompanyId }),
    onSuccess: () => {
      message.success('已提交审批')
      queryClient.invalidateQueries({ queryKey: ['fin', 'cash-out'] })
      queryClient.invalidateQueries({ queryKey: ['fin', 'cash-out', 'detail', selectedRecord?.id] })
    },
    onError: (error) => {
  ...
  const openDetail = (record: FinCashOutRecord) => {
  ...
  const columns: ColumnsType<FinCashOutRecord> = useMemo(
    () =>
      addResizableToColumns<FinCashOutRecord>([
        {
          title: 'ID',
          dataIndex: 'id',
          key: 'id',
        ...
        <Form
          ...
          onFinish={(values) => {
            ...
            createMutation.mutate({
              ...
              payee_id: values.payee_id ? Number(values.payee_id) : undefined,
        ...
        >
          ...
          <Form.Item label="收款方类型" name="payee_type" rules={[{ required: true, message: '请选择类型' }]}>
            <Select
              ...
              onChange={(v) => {
                setPayeeType(v)
                createForm.setFieldValue('payee_id', undefined)
              }}
            />
          </Form.Item>
          {payeeType === 'supplier' ? (
            <Form.Item label="供应商" name="payee_id" rules={[{ required: true, message: '请选择供应商' }]}>
              <Select
                showSearch
                placeholder="请选择供应商"
            ...
            <Form.Item label="收款方ID" name="payee_id">
          ...
          <Form.Item label="付款日期" name="cash_date" rules={[{ required: true, message: '请选择日期' }]}>
